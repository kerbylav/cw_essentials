	...
	*/
	...
	this.checkFolding = function() {
		$(".folding").each(function(index, element){
			if ($(element).parent(".comment-actions").parent(".comment").nextAll(".comment-wrapper").length == 0) {
				$(element).hide();
			} else {
				$(element).show();
			}
	...
	this.expandComment = function(folding) {
		$(folding).removeClass("folded").html(ls.lang.get('comment_fold')).parent().parent().nextAll(".comment-wrapper").show();
	}
	
	this.collapseComment = function(folding) {
		$(folding).addClass("folded").html(ls.lang.get('comment_unfold')).parent().parent().nextAll(".comment-wrapper").hide();
	}
